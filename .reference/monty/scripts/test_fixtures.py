...
Holds external-function implementations for iter mode tests, injected into
CPython test globals via the `exported_globals` dict at the bottom of this
file. They mirror `dispatch_external_call` in the Rust runner so iter-mode
tests produce identical results on both sides.
