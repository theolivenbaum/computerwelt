//! Downstream-style PyO3 fixture that validates the public filesystem capsule ABI.
...
impl SeededRandomFs {
    ...
    fn content_for(&self, path: &Path) -> Option<Vec<u8>> {
        ...
        match path {
            ...
            "/random.txt" => Some(expected_random_text(self.seed, path).into_bytes()),
            "/nested/data.txt" => {
...
#[pyfunction]
fn expected_random_text(seed: u64, path: &str) -> String {
    format!(
        "seed={seed}\npath={path}\nvalue={:016x}\n",
        value_for(seed, path)
    )
...
#[pymodule]
fn bashkit_random_fs(module: &Bound<'_, PyModule>) -> PyResult<()> {
    module.add_function(wrap_pyfunction!(create_random_filesystem_capsule, module)?)?;
    module.add_function(wrap_pyfunction!(expected_random_text, module)?)?;
    Ok(())
}
