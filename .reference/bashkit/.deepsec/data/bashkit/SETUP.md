# Agent setup for `bashkit`

This is a deepsec scanning workspace. Project `bashkit` was just registered
(target: `..`). Setup is incomplete — `data/bashkit/INFO.md`
...
2. **Fill in `data/bashkit/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id bashkit
pnpm deepsec process --project-id bashkit
```
